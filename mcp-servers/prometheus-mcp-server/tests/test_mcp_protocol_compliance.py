...
from prometheus_mcp_server.server import (
    make_prometheus_request, config,
...
class TestMCPServerConfiguration:
    ...
    def test_server_config_validation(self):
        ...
        assert mcp_config.mcp_bind_host == "127.0.0.1"
        assert mcp_config.mcp_bind_port == 8080
...
class TestMCPProtocolVersioning:
    ...
    def test_server_capabilities(self):
        ...
        # Test that the server only supports HTTP transport
        # The server now only supports HTTP transport for JSON-RPC
        pass
